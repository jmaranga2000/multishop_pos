...
export function buildThermalReceiptHtml(data: ThermalReceiptData) {
  ...
  return [
    ...
    `<div style="text-align:center"><div style="font-size:14px;font-weight:700;text-transform:uppercase">${escapeHtml(safeText(data.businessName) || "Receipt")}</div><div style="margin-top:6px;font-weight:700;color:${etims ? "#1d4ed8" : "#334155"}">${etims ? "eTIMS / VAT RECEIPT" : "NORMAL RECEIPT"}</div></div>`,
    `<div style="margin-top:10px;border-top:1px dashed #cbd5e1;padding-top:8px"><div style="display:flex;justify-content:space-between"><span>Receipt</span><strong>${escapeHtml(data.receiptNumber)}</strong></div><div style="display:flex;justify-content:space-between"><span>Date</span><span>${escapeHtml(formatDate(data.occurredAt))}</span></div></div>`,
...
export function ReceiptPdfDocument({ data }: { data: ThermalReceiptData }) {
  const etims = isEtims(data);
  return <Document><Page size={{ width: 260, height: "auto" }} style={styles.page}><View style={styles.center}><Text style={styles.bold}>{safeText(data.businessName) || "Receipt"}</Text><Text>{etims ? "eTIMS / VAT RECEIPT" : "NORMAL RECEIPT"}</Text></View><View style={styles.divider}><View style={styles.row}><Text>Receipt</Text><Text>{data.receiptNumber}</Text></View><View style={styles.row}><Text>Date</Text><Text>{formatDate(data.occurredAt)}</Text></View></View><View style={styles.divider}>{data.items.map((item, index) => <View key={`${item.name}-${index}`} style={styles.row}><Text>{item.name} × {item.quantity}</Text><Text>{formatAmount(item.lineTotalMinor)}</Text></View>)}</View><View style={styles.divider}>{etims ? <><View style={styles.row}><Text>Taxable amount</Text><Text>{formatAmount(data.taxableMinor ?? data.subtotalMinor)}</Text></View><View style={styles.row}><Text>VAT {data.vatRate ?? 0}%</Text><Text>{formatAmount(data.taxMinor)}</Text></View></> : <View style={styles.row}><Text>Subtotal</Text><Text>{formatAmount(data.subtotalMinor)}</Text></View>}<View style={styles.row}><Text style={styles.bold}>TOTAL</Text><Text style={styles.bold}>{formatAmount(data.grandTotalMinor)}</Text></View><View style={styles.row}><Text>Payment</Text><Text>{data.paymentMethod}</Text></View></View>{etims ? <View style={styles.divider}><Text style={styles.bold}>eTIMS INFORMATION</Text><Text>Status: {safeText(data.etims?.status) || "Not fiscalized"}</Text>{safeText(data.etims?.officialInvoiceNumber) ? <Text>Invoice: {data.etims?.officialInvoiceNumber}</Text> : null}{safeText(data.etims?.fiscalDocumentNumber) ? <Text>Fiscal document: {data.etims?.fiscalDocumentNumber}</Text> : null}{safeText(data.etims?.controlCode) ? <Text>Control code: {data.etims?.controlCode}</Text> : null}</View> : null}{data.qrCodeDataUrl ? <Image src={data.qrCodeDataUrl} style={styles.qr} /> : null}</Page></Document>;
}
