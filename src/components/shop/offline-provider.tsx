...
export function OfflineProvider({ children, shopId, shopName }: { children: React.ReactNode; shopId: string; shopName: string }) {
  ...
  const lastSyncAt = useLiveQuery(() => getLastSynchronizationTime(shopId), [shopId], null);
  const pendingCount = useLiveQuery(() => countPendingSynchronizationRecords(shopId), [shopId], 0) ?? 0;
  const conflictCount = useLiveQuery(() => countSynchronizationConflicts(shopId), [shopId], 0) ?? 0;

  const syncNow = useCallback(async (options: SyncOptions = {}) => {
    if (!navigator.onLine || syncing) return;
    setSyncing(true);
    try {
      const result = await syncPendingSales({ ...options, shopId });
  ...
  useEffect(() => {
    ...
    if (navigator.onLine) void (async () => { try { await bootstrapOfflineData(); await syncNow(); } catch { /* cached data remains usable */ } })();
    return () => {
