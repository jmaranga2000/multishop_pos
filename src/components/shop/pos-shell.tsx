...
type CartLine = { productId: string; name: string; sku: string; quantity: number; unitPriceMinor: number; unitCostMinor: number; available: number };
...
export function PosShell() {
  ...
  function add(entry: (typeof products)[number]) {
    ...
    setCart((current) => {
      ...
      if (existing) {
        if (existing.quantity >= entry.projectedQuantity) { toast.warning(`Only ${entry.projectedQuantity} units are projected to be available`); return current; }
        return current.map((item) => item.productId === product.id ? { ...item, quantity: item.quantity + 1 } : item);
      }
      return [...current, { productId: product.id, name: product.name, sku: product.sku, quantity: 1, unitPriceMinor: entry.sellingPriceMinor, unitCostMinor: entry.costPriceMinor, available: entry.projectedQuantity }];
    });
  }
  ...
  function changeQuantity(productId: string, delta: number) {
    setCart((current) => current.flatMap((item) => {
      ...
      if (quantity > item.available) { toast.warning(`Only ${item.available} units are projected to be available`); return [item]; }
  ...
  async function checkout() {
    ...
    setProcessing(true);
    try {
      const sale = await createLocalSale({ shopId, paymentMethod: "CASH", amountPaidMinor: receivedMinor, items: cart.map((item) => ({ productId: item.productId, productName: item.name, sku: item.sku, quantity: item.quantity, unitPriceMinor: item.unitPriceMinor, unitCostMinor: item.unitCostMinor })) });
    ...
    <div className="pos-layout">
      <section className="min-w-0">
        ...
        {filtered.length ? <div className="product-grid">{filtered.map((entry) => {
          const product = entry.product!; const status = getStockStatus(entry.projectedQuantity, entry.reorderLevel, entry.criticalLevel);
          return <button key={entry.id} onClick={() => add(entry)} disabled={entry.projectedQuantity <= 0} className="product-card surface rounded-2xl p-4 disabled:cursor-not-allowed disabled:opacity-55">
            <div><div className="flex items-start justify-between gap-2"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-blue-50 font-black text-blue-700">{product.name.slice(0,2).toUpperCase()}</div><Badge tone={status === "IN_STOCK" ? "success" : status === "LOW_STOCK" ? "warning" : "danger"}>{entry.projectedQuantity} left</Badge></div><p className="mt-3 line-clamp-2 font-bold text-slate-900">{product.name}</p><p className="mt-1 text-xs text-slate-500">{product.sku}</p></div><p className="mt-3 text-lg font-black text-[#173b89]">{formatMoney(fromMinorUnits(entry.sellingPriceMinor))}</p>
          </button>;
      ...
      <Card className="cart-panel overflow-hidden">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4">{cart.length ? <div className="space-y-3">{cart.map((item) => <div key={item.productId} className="rounded-xl border border-slate-200 p-3"><div className="flex justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-bold">{item.name}</p><p className="text-xs text-slate-500">{formatMoney(fromMinorUnits(item.unitPriceMinor))} each</p></div><button onClick={() => setCart((current)=>current.filter((line)=>line.productId!==item.productId))}><Trash2 className="h-4 w-4 text-red-500"/></button></div><div className="mt-3 flex items-center justify-between"><div className="flex items-center gap-2"><button className="rounded-lg border p-1" onClick={()=>changeQuantity(item.productId,-1)}><Minus className="h-4 w-4"/></button><span className="w-7 text-center text-sm font-bold">{item.quantity}</span><button className="rounded-lg border p-1" onClick={()=>changeQuantity(item.productId,1)}><Plus className="h-4 w-4"/></button></div><p className="font-black">{formatMoney(fromMinorUnits(item.quantity*item.unitPriceMinor))}</p></div></div>)}</div> : <div className="flex h-full min-h-52 flex-col items-center justify-center text-center"><ShoppingCart className="h-10 w-10 text-slate-200"/><p className="mt-3 font-bold text-slate-700">Your cart is empty</p><p className="mt-1 text-sm text-slate-400">Select a product to begin.</p></div>}</div>
