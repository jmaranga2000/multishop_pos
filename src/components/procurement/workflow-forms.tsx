...
export function RequisitionForm({ action, shops, suppliers, products, fixedShopId }: { action: FormAction; shops?: Shop[]; suppliers: Supplier[]; products: Product[]; fixedShopId?: string }) {
  ...
  return <form action={action} className="space-y-3">
    ...
    <div className="grid gap-2 sm:grid-cols-[1fr_110px_auto]"><select value={productId} onChange={(event) => setProductId(event.target.value)} className="rounded-lg border bg-white px-3 py-2"><option value="">Select product</option>{products.map((product) => <option key={product.id} value={product.id}>{product.name} {product.sku ? `(${product.sku})` : ""}</option>)}</select><Input type="number" min="0.01" step="0.01" value={quantity} onChange={(event) => setQuantity(event.target.value)} /><Button type="button" variant="secondary" onClick={addItem}>Add item</Button></div>
