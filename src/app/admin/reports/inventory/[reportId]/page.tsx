  ...
  return (
    <>
      ...
      <Card className="mt-4 overflow-hidden">
        <div className="overflow-x-auto"><table className="data-table"><thead><tr><th>Product</th><th>Shop</th><th>Opening</th><th>Added</th><th>Sold</th><th>Closing</th><th>Daily avg.</th><th>Days left</th><th>Status</th></tr></thead><tbody>{report.items.map((item: typeof report.items[number]) => <tr key={item.id}><td><p className="font-bold">{item.product.name}</p><p className="text-xs text-slate-500">{item.product.sku}</p></td><td>{item.shop.name}</td><td>{item.openingQuantity}</td><td>{item.quantityAdded}</td><td>{item.quantitySold}</td><td className="font-black">{item.closingQuantity}</td><td>{Number(item.averageDailySales).toFixed(1)}</td><td>{item.estimatedDaysRemaining === null ? "No recent sales" : Number(item.estimatedDaysRemaining).toFixed(1)}</td><td><Badge tone={item.stockStatus === "IN_STOCK" ? "success" : item.stockStatus === "LOW_STOCK" ? "warning" : "danger"}>{item.stockStatus.replaceAll("_", " ")}</Badge></td></tr>)}</tbody></table></div>
      </Card>
    </>
  );
}
