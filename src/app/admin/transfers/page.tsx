...
import { TransferCreateForm } from "@/components/admin/transfer-create-form";

export const dynamic = "force-dynamic";

export default async function TransfersPage() {
  const user = await requireAdmin();
  const { shops, productsByShop, transfers } = await getTransferManagementData(user.businessId);
  ...
  return (
    <>
      ...
      <div className="grid gap-5 xl:grid-cols-[1fr_390px]">
        ...
        <TransferCreateForm shops={shops} productsByShop={productsByShop} />
      </div>
    </>
  );
}
