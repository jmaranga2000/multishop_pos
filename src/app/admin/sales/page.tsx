  ...
  return (
    <>
      ...
      <Card className="overflow-hidden">
        ...
        {sales.length ? (
          <div className="overflow-x-auto"><table className="data-table"><thead><tr><th>Receipt</th><th>Shop</th><th>Date</th><th>Items</th><th>Payment</th><th>Total</th><th>Sync</th></tr></thead><tbody>{sales.map((sale) => <tr key={sale.id}><td className="font-mono text-xs font-bold">{sale.receiptNumber}</td><td>{sale.shop.name}</td><td>{sale.occurredAt.toLocaleString("en-KE")}</td><td>{sale._count.items}</td><td>{sale.payments.map((payment) => payment.method).join(", ")}</td><td className="font-black">{formatMoney(sale.total.toString(), business.currency)}</td><td>{sale.isOffline ? <Badge tone={sale.syncedAt ? "info" : "warning"}>{sale.syncedAt ? "Offline synced" : "Pending"}</Badge> : <Badge tone="success">Online</Badge>}</td></tr>)}</tbody></table></div>
