...
import { verifyBiometricAuthentication } from "@/services/shop/biometric-service";
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(
      await verifyBiometricAuthentication(shopUser, {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Unable to verify fingerprint." },
      { status: 400 },
    );
  }
}
