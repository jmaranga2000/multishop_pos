...
export async function reconcileStockAlert(tx: Tx, input: {
  ...
  if (shouldNotifyAdmin && (inAppEnabled || pushEnabled || emailEnabled)) {
    const label = type === "LOW_STOCK" ? "Low stock" : type === "CRITICAL_STOCK" ? "Critical stock" : "Out of stock";
    const tone = type === "LOW_STOCK" ? "amber" : "red";
  ...
  try {
    await queueAutomaticSupplierRestockRequests(tx, {
      businessId: input.businessId,
      shopId: input.shopId,
    ...
    // Stock movement must still commit if a supplier-notification record cannot be queued.
    console.error(`Unable to queue supplier restock email for ${input.productId}:`, error);
  }
}